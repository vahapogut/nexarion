...
export function createHTTPServer(config: HTTPServerConfig) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url === '/registry/search' || url.startsWith('/registry/search?q=')) {
      const query =
        new URL(url, `http://${config.host || 'localhost'}`).searchParams.get('q') || '';
      const agents = nexarion
        .listAgents()
        .filter(
          (a) =>
            !query ||
            a.card.name.toLowerCase().includes(query.toLowerCase()) ||
      ...
      const results = await Promise.all(
        agents.map(async (a) => {
          try {
            ...
            return { name: a.card.name, status: 'down' };
          }
        }),
      );
      res.setHeader('Content-Type', 'application/json');
            ...
            res.end(
              JSON.stringify({
                ...
                result: {
                  ...
                  serverInfo: { name: 'nexarion-server', version: '0.6.0' },
                },
                id,
              }),
            );
